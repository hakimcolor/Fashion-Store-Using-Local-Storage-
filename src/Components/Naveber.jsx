...
const Naveber = () => {
  ...
  const navStyle = (path) => {
    ...
    return `relative flex items-center gap-1.5 transition-all duration-300 font-medium text-sm cursor-pointer pb-1.5 ${
      isActive
        ? 'font-semibold text-[#8FA28A] after:absolute after:bottom-0 after:left-0 after:w-full after:h-0.5 after:bg-[#8FA28A] after:rounded-full'
        : 'text-gray-600 dark:text-gray-300 hover:text-[#8FA28A] dark:hover:text-[#8FA28A] after:absolute after:bottom-0 after:left-0 after:w-0 after:h-0.5 after:bg-[#8FA28A] after:rounded-full after:transition-all after:duration-300 hover:after:w-full'
    }`;
  };
  ...
  return (
    <header
      className={`sticky top-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'shadow-xl backdrop-blur-3xl bg-white/90 dark:bg-[#2d3d35]/90 border-b border-gray-200 dark:border-gray-700'
          : 'backdrop-blur-xl bg-white/80 dark:bg-[#2d3d35]/80 border-b border-gray-100 dark:border-gray-800'
      }`}
    >
      <div className="max-w-[95%] mx-auto">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <nav className="hidden md:flex items-center gap-6 dmsans font-bold">
            {menus.map((menu) => (
              <Link
                ...
                style={
                  location.pathname === menu.path ? { color: '#8FA28A' } : {}
                }
              >
                {menu.icon}
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {dark ? (
                <FaSun className="text-[#C8A96B] text-base" />
              ) : (
            ...
            >
              {open ? (
                <FaTimes className="hover:text-[#C8A96B] transition-all duration-300" />
              ) : (
