  ...
  return (
    <div className="w-full max-w-[95%] mx-auto py-10">
      ...
      <div className="flex items-center gap-2 text-sm dmsans mb-8">
        <Link
          to="/"
          className="text-gray-500 dark:text-gray-400 hover:text-[#8FA28A] transition-colors"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
        ...
        <div
          data-aos="fade-right"
          className="overflow-hidden rounded-3xl bg-gradient-to-br from-[#F7F4ED] to-gray-100 dark:from-gray-700 dark:to-gray-900 shadow-xl h-[90%] sm:h-96 lg:h-[720px]"
        >
          <img
            src={product.image}
            alt={product.name}
        ...
        <div className="space-y-5" data-aos="fade-left">
          <div className="flex flex-wrap items-center gap-3">
            <span className="bg-[#8FA28A] text-white px-4 py-1.5 rounded-full text-sm dmsans font-semibold">
              {product.category}
            </span>
            <div className="flex items-center gap-2 bg-[#C7D3C0] dark:bg-[#C8A96B]/20 px-3 py-1.5 rounded-full">
              <FaStar className="text-[#C8A96B] text-sm" />
              <span className="text-sm font-bold text-gray-800 dark:text-[#C8A96B]">
                {product.rating}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="text-xs font-semibold bg-[#C7D3C0] dark:bg-[#8FA28A]/20 text-green-700 dark:text-[#8FA28A] px-3 py-1 rounded-full dmsans">
              🚚 Free Delivery
            </span>
          </div>
          ...
          <p className="dmsans text-sm">
            ...
            <span
              className={`font-bold ${product.inStock ? 'text-[#8FA28A]' : 'text-[#C8A96B]'}`}
            >
              {product.inStock
                ...
                {product.colors.map((color, idx) => (
                  <button
                    ...
                    className={`w-9 h-9 rounded-full border-[3px] transition-all cursor-pointer hover:scale-110 ${
                      selectedColor === color
                        ? 'border-[#8FA28A] scale-110 ring-2 ring-[#8FA28A]/30'
            ...
            <div>
              <p className="text-xs uppercase tracking-[2px] text-gray-500 dark:text-gray-400 mb-2 dmsans flex items-center gap-1">
                ...
                <span
                  ...
                  className="cursor-help text-[#8FA28A] text-xs border border-[#8FA28A] rounded-full w-4 h-4 inline-flex items-center justify-center leading-none"
                >
                  ?
                </span>
              </p>
                ...
                {product.sizes.map((size, idx) => (
                  <button
                    ...
                    className={`w-10 h-10 rounded-xl border font-semibold transition-all cursor-pointer text-sm hover:scale-105 ${
                      selectedSize === size
                        ? 'bg-[#8FA28A] text-white border-[#8FA28A] shadow-md'
                        : 'border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-[#8FA28A] hover:text-white hover:border-[#8FA28A]'
                    }`}
                  >
                    {size}
                  </button>
          ...
          <div className="rounded-2xl border border-gray-100 dark:border-gray-700 bg-[#F7F4ED]/50 dark:bg-[#8FA28A]/10 px-5 py-4">
            <p className="dmsans text-xs uppercase tracking-widest text-gray-500 dark:text-gray-400 mb-1">
              {tr('detail_total', lang)}
            </p>
            <h2 className="text-2xl font-bold flex items-center text-[#8FA28A]">
              <TbCurrencyTaka className="text-2xl" />
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 lg:hidden bg-white dark:bg-[#2d3d35] border-t border-gray-200 dark:border-gray-700 px-4 py-3 flex items-center gap-3 shadow-2xl">
        <div className="flex-1">
          <p className="dmsans text-xs text-gray-500 dark:text-gray-400 truncate">
            {product.name}
          </p>
          <p className="font-bold text-[#8FA28A] flex items-center text-lg">
            <TbCurrencyTaka />
            {totalAmount.toLocaleString()}
          </p>
        </div>
