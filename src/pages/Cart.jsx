...
const Cart = ({ onClose }) => {
    ...
    return (
      ...
      >
        <div className="relative w-20 h-20">
          ...
          />
          <FiShoppingCart className="absolute inset-0 m-auto text-4xl text-[#8FA28A]" />
        </div>
  ...
  const CartItem = ({ item, compact = false }) => (
    <div className="flex items-center gap-3 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl p-3 shadow-sm hover:shadow-md transition-all duration-300 group relative overflow-hidden">
      ...
      <div className="flex-1 min-w-0">
          ...
          {[item.selectedColor, item.selectedSize].map(
            (attr, idx) =>
              attr && (
                <span
                  key={idx}
                  className="text-xs bg-[#F7F4ED] dark:bg-[#8FA28A]/30 px-2 py-0.5 rounded-full text-[#8FA28A]"
                >
                  {attr}
                </span>
              )
        ...
        <p className="text-sm font-semibold flex items-center mt-1 text-[#8FA28A]">
      ...
      <div className="flex flex-col items-end gap-2 shrink-0">
        <button
          ...
          className="w-8 h-8 flex items-center justify-center rounded-full bg-red-50 dark:bg-red-900/20 text-[#C8A96B] hover:bg-[#C8A96B] hover:text-white transition-all duration-200 cursor-pointer"
        >
        ...
        <div className="flex items-center gap-1 border border-gray-200 dark:border-gray-600 rounded-full px-1.5 py-0.5">
          <button
            ...
            className="w-5 h-5 flex items-center justify-center rounded-full border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:bg-[#C8A96B] hover:text-white hover:border-[#C8A96B] active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200 cursor-pointer"
          >
          ...
          <button
            onClick={() => updateQuantity(item.id, 1)}
            className="w-5 h-5 flex items-center justify-center rounded-full border text-white active:scale-95 transition-all duration-200 cursor-pointer bg-[#8FA28A] border-[#8FA28A]"
          >
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden h-full">
      ...
      <div className="shrink-0 border-t border-gray-100 dark:border-gray-700 px-4 py-4 bg-white dark:bg-[#2d3d35]">
        <Summary compact />
      </div>
    </div>
  );
