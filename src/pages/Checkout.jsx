...
const Field = ({ label, error, children }) => (
  <div className="flex flex-col gap-1.5">
    ...
    {children}
    {error && <p className="dmsans text-xs text-red-500">{error}</p>}
  </div>
);

const inputCls =
  'w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white dmsans text-sm outline-none focus:border-[#155dfc] focus:ring-2 focus:ring-[#155dfc]/20 transition-all duration-300';
  ...
  if (cart.length === 0 && !placed) {
    return (
      <div className="w-[95%] mx-auto py-24 flex flex-col items-center gap-4 text-center">
        ...
        <Link
          to="/products"
          className="inline-flex items-center gap-2 mt-2 bg-[#155dfc] text-white dmsans font-semibold px-6 py-3 rounded-xl hover:bg-blue-700 transition-all"
        >
    ...
    return (
      ...
      >
        <div className="w-20 h-20 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center">
          <FiCheck className="text-4xl text-green-500" strokeWidth={3} />
        </div>
        ...
        <div className="flex flex-wrap gap-3 justify-center mt-2">
          <Link
            to="/products"
            className="inline-flex items-center gap-2 bg-[#155dfc] text-white dmsans font-semibold px-6 py-3 rounded-xl hover:bg-blue-700 transition-all"
          >
            {tr('home_view_all', lang)} <HiArrowRight />
          </Link>
          <Link
  ...
  return (
    <div className="w-[95%] mx-auto py-10">
      ...
      <div className="flex items-center gap-2 text-sm dmsans mb-8 text-gray-500 dark:text-gray-400">
        <Link to="/" className="hover:text-[#155dfc] transition-colors">
          {tr('nav_home', lang)}
        </Link>
        <span>/</span>
        <Link to="/cart" className="hover:text-[#155dfc] transition-colors">
          {tr('cart_title', lang)}
        </Link>
        <span>/</span>
        <span className="text-[#155dfc]">{tr('checkout_title', lang)}</span>
      </div>
      ...
      <form onSubmit={handleSubmit} noValidate>
        <div className="flex flex-col lg:flex-row gap-8 items-start">
                ...
                {PAYMENT_METHODS.map((m) => (
                  <button
                    ...
                    className={`flex items-center gap-3 p-3 rounded-xl border-2 text-left transition-all duration-200 cursor-pointer ${
                      payment === m.id
                        ? 'border-[#155dfc] bg-blue-50 dark:bg-blue-900/20'
                        : 'border-gray-200 dark:border-gray-600 hover:border-[#155dfc]/50'
                    }`}
                  >
                    <span className="text-2xl">{m.icon}</span>
                    <span
                      className={`dmsans text-sm font-semibold ${payment === m.id ? 'text-[#155dfc]' : 'text-gray-700 dark:text-gray-300'}`}
                    >
                      {lang === 'bn' ? m.bn : m.en}
                    </span>
                    {payment === m.id && (
                      <FiCheck
                        className="ml-auto text-[#155dfc] shrink-0"
                        strokeWidth={3}
                      />
                    )}
                  </button>
          ...
          >
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-5 shadow-md">
              ...
              <div className="border-t border-dashed border-gray-200 dark:border-gray-600 mt-4 pt-4 space-y-2.5">
                ...
                <div className="border-t border-gray-100 dark:border-gray-700 pt-2.5 flex justify-between items-center">
                  ...
                  <span className="arbutus-slab text-lg font-bold text-[#155dfc] flex items-center">
            ...
            <button
              type="submit"
              className="w-full flex items-center justify-center gap-2 bg-[#155dfc] hover:bg-blue-700 text-white font-semibold py-4 rounded-2xl transition-all duration-300 shadow-lg active:scale-[.98] cursor-pointer dmsans"
            >
